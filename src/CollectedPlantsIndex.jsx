    ...
    <div id="collected-plants-index">
      ...
      {props.collectedPlants.map((collectedPlant) => (
        <div key={collectedPlant.id}>
          ...
          <button onClick={() => props.onShowCollectedPlant(collectedPlant)}>Edit Information</button>
          <button onClick={() => props.onShowOriginalPlant(collectedPlant.plant)}>Original Plant Information</button>
        </div>
      ))}
    </div>
  );
